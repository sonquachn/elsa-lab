...
# Security group for docker build
resource "aws_security_group" "docker_elsa" {
  name   = "docker-elsa-lab"
  vpc_id = aws_vpc.this.id
...
# Security group traffic rules
resource "aws_security_group_rule" "elsa_ingress_public_22" {
  security_group_id = aws_security_group.docker_elsa.id
  type              = "ingress"
  from_port         = 22
  to_port           = 22
  protocol          = "tcp"
...
resource "aws_security_group_rule" "elsa_egress_public" {
  security_group_id = aws_security_group.docker_elsa.id
  type              = "egress"
  from_port         = 0
  to_port           = 0
